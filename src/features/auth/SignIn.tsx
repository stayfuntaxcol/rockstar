  ...
  return (
    <div className=\"max-w-sm mx-auto mt-24 bg-white rounded-2xl shadow-card p-6\">
      ...
      <form onSubmit={submit} className=\"space-y-3\">
        ...
        <input className=\"w-full rounded-lg border p-2\" type=\"password\" placeholder=\"Wachtwoord\" value={pw} onChange={e=>setPw(e.target.value)}/>
        {err && <div className=\"text-red-600 text-sm\">{err}</div>}
        <button className=\"w-full rounded-lg bg-brand text-white py-2 font-semibold\">Log in</button>
      </form>
    </div>
  );
}
