...
export default function SignIn() {
  ...
  useEffect(()=>{ const off=onAuthStateChanged(auth,u=>{ if(u) go("/pipeline"); }); return off; },[]);
  function go(path:string){ if(!path.startsWith("/")) path="/"+path; window.location.hash="#"+path; }

  async function ensureRoles(uid:string){
    const ref=doc(db,"roles",uid); const snap=await getDoc(ref);
    if(!snap.exists()) await setDoc(ref,{ viewer:true, rm:true },{ merge:true });
  }
  function valid(p:string){ return p.length>=8 && /\d/.test(p); }
  ...
  async function submit(e:React.FormEvent){ 
    ...
    try{
      if(mode==="login"){
        ...
        await ensureRoles(res.user.uid); go("/pipeline");
      }else{
        if(!valid(pass)) throw new Error("Min. 8 tekens en 1 cijfer.");
      ...
      <div className="w-full max-w-sm bg-white rounded-2xl border shadow-card p-6 space-y-4">
        ...
        <form onSubmit={submit} className="space-y-3">
          <div>
            ...
            <input className="w-full border rounded-lg px-3 py-2" type="email" required value={email} onChange={e=>setEmail(e.target.value)} autoComplete="email"/>
          </div>
          <div>
            <label className="block text-sm text-slate-600 mb-1">Wachtwoord</label>
            <input className="w-full border rounded-lg px-3 py-2" type="password" required value={pass} onChange={e=>setPass(e.target.value)} autoComplete={mode==="login"?"current-password":"new-password"} placeholder={mode==="register"?"Min. 8 tekens, 1 cijfer":""}/>
          </div>
        ...
        {mode==="login" ? (
          <div className="text-sm flex items-center justify-between">
            <button className="underline" onClick={resetPw}>Wachtwoord vergeten?</button>
        ...
        <p className="text-xs text-slate-500">Na eerste login wordt <code>roles/&lt;uid&gt;</code> gezet met viewer+rm.</p>
      </div>
    </div>
  );
}
