...
export default function SignIn() {
  ...
  // Als je al ingelogd bent: door naar dashboard
  useEffect(() => {
  ...
  function validatePassword(p: string) {
    // eenvoudige policy: min 8 chars, 1 cijfer
    ...
    try {
      if (mode === "login") {
        ...
        if (!validatePassword(pass)) {
          throw new Error("Wachtwoord te zwak (min. 8 tekens en minimaal 1 cijfer).");
        }
  ...
  async function doReset() {
    setErr("");
    try {
      if (!email) throw new Error("Vul eerst je e-mail in.");
      await sendPasswordResetEmail(auth, email.trim());
      alert("Wachtwoord reset link verzonden.");
    } catch (e: any) {
      setErr(e?.message || String(e));
    }
  }
    ...
    <div className="min-h-screen grid place-items-center bg-bg p-6">
      <div className="w-full max-w-sm bg-white rounded-2xl border shadow-card p-6 space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <button
            disabled={loading}
            className="w-full rounded-lg bg-brand text-white py-2 font-medium"
            type="submit"
          >
        ...
        <p className="text-xs text-slate-500">
          Tip: stel per gebruiker rollen in via Firestore → <code>roles/&lt;uid&gt;</code>. Standaard zetten we bij eerste login <code>viewer:true</code> en <code>rm:true</code>.
        </p>
      </div>
    </div>
  );
